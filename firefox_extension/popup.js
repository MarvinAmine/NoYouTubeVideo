...
if (typeof browser === 'undefined') {
  ...
  async function getPreferredActivity() {
    try {
      const result = await browser.storage.local.get(['activityStats']);
  ...
  async function updateActivityStats(activity) {
    try {
      ...
      await browser.storage.local.set({ activityStats: stats });
    } catch (error) {
  ...
  async function getActivityChoices() {
    try {
      const result = await browser.storage.local.get(['customActivityChoices']);
  ...
  async function saveActivityChoices(choices) {
    try {
      await browser.storage.local.set({ customActivityChoices: choices });
    } catch (error) {
  ...
  async function showChoices(activity) {
    ...
    currentActivityChoices.forEach((choice, index) => {
      ...
      setRedirectButton.addEventListener('click', async (e) => {
        ...
        try {
          await browser.storage.local.set({ redirectUrl: choice.url });
    ...
    restoreButton.addEventListener('click', async () => {
      showCustomAlert(
        'Are you sure you want to restore default activities? This will remove any custom activities.',
        async () => {
  ...
  async function initializeBypassButton() {
    ...
    const result = await browser.storage.local.get(['bypassCount', 'lastBypassDate']);
    let bypassCount = result.bypassCount || 0;
    const lastBypassDate = result.lastBypassDate;
  
    // Reset bypass count if it's been 3 days
    if (lastBypassDate) {
      const daysSinceLastBypass =
        (Date.now() - new Date(lastBypassDate).getTime()) / (1000 * 60 * 60 * 24);
      if (daysSinceLastBypass >= 3) {
        bypassCount = 0;
        await browser.storage.local.set({ bypassCount: 0 });
      }
    }
      ...
      try {
        ...
        if (tabs[0] && result.lastYoutubeUrl) {
          ...
          if (result.lastYoutubeUrl) {
            await browser.tabs.create({ url: result.lastYoutubeUrl });
          }
        }
      } catch (error) {
  ...
  async function checkFirstVisit() {
    try {
      const result = await browser.storage.local.get(['hasVisited']);
  ...
  async function markAsVisited() {
    try {
      await browser.storage.local.set({ hasVisited: true });
    } catch (error) {
      ...
      showOptionsBtn.addEventListener('click', async () => {
        try {
          ...
          await browser.tabs.create({
            url: browser.runtime.getURL('redirect.html')
          });
          // Close the popup
          window.close();
        } catch (error) {

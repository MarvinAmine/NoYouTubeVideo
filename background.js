...
async function handleTabUpdate(tabId, changeInfo, tab) {
  ...
  if (changeInfo.url && isBlockedYouTubeUrl(changeInfo.url)) {
    ...
    try {
      ...
      const result = await chrome.storage.local.get(['redirectUrl', 'tempYoutubeAccess', 'lastYoutubeUrl']);
      
      // Store the current URL for potential bypass
      await chrome.storage.local.set({ lastYoutubeUrl: changeInfo.url });

      // If user chose to visit YouTube, allow it
      const currentUrlId = getYouTubeUrlIdentifier(changeInfo.url);
      const storedUrlId = result.lastYoutubeUrl ? getYouTubeUrlIdentifier(result.lastYoutubeUrl) : null;
...
async function resetDailyStats() {
  try {
    ...
    if (stats.lastReset !== today) {
      ...
      await chrome.storage.local.set({ timeStats: stats });
    }
  } catch (error) {
...
async function init() {
  ...
  await chrome.storage.local.set({ tempYoutubeAccess: false });
}
